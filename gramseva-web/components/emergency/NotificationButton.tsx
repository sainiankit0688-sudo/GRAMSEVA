...
const NOTIFICATION_PREF_KEY = 'gs_emergency_notifications';
const NOTIFICATIONS_TABLE = 'emergency_notifications';
...
export default function NotificationButton({ itemId, label = 'Get Alerts' }: NotificationButtonProps) {
  ...
  return (
    <button
      onClick={toggle}
      className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium border transition-colors focus:outline-none focus:ring-2 focus:ring-red-400 ${
        subscribed
          ? 'bg-red-50 border-red-200 text-red-700'
          : 'bg-white border-gray-200 text-gray-600 hover:bg-red-50 hover:border-red-200'
      }`}
      aria-label={subscribed ? 'Unsubscribe from alerts' : 'Get alerts for updates'}
      aria-pressed={subscribed}
    >
      <svg className="w-3.5 h-3.5" fill={subscribed ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
